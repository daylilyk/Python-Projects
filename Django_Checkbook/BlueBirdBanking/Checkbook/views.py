...
# This function wil render the Home page when requested
def home(request):
...
def balance(request, pk):
    ...
    transactions = Transaction.Transactions.filter(account=pk)  # Retrieve all the account's transactions
    current_total = account.initial_deposit  # Create account total variable, starting with the initial deposit value
...
def transaction(request):
    ...
    # Pass content to the template in directory
    content = {'form': form}  # Saves content to the template as a dictionary
    # Adds content of form to page
