...
def first_run(self):
    data = ('John', 'Doe', 'John Doe', '111-111-1111', 'jdoe@gmail.com', 'Python')
    conn = sqlite3.connect('studentTracking.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_studentTracking(col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(data))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()            
        cursor.execute("""SELECT col_fname, col_lname, col_phone, col_email, col_course FROM tbl_studentTracking WHERE col_fullname = (?)""",[value])
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0):
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existence of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_studentTracking (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_course))
...
def onRefresh(self):
    ...
    with conn:
        ...
        i = 0
        while i < count:
            cur.execute("""SELECT col_fullname FROM tbl_studentTracking""")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and(len(var_course) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_studentTracking WHERE col_course = '{}'""".format(var_course))
            count3 = cur.fetchone()[0]
            print(count3)
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}), ({}), and ({}) will be implemented for ({}), \nOk?".format(var_phone, var_email, var_course, var_value))
                print(response)
                if response:
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_studentTracking SET col_phone = '{0}',col_email = '{1}',col_course ='{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
